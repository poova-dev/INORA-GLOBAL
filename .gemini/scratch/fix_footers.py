#!/usr/bin/env python3
"""Replace minimal footers in INORA subpages with the full professional footer."""
import re
import os

ROOT = "/Users/admin/Documents/Flyggo/INORA GLOBAL"
...
# Pattern: minimal footer with just footer-bottom
MINIMAL_FOOTER_PATTERN = re.compile(
    r'<footer class="site-footer">\s*<div class="container">\s*<div class="footer-bottom">.*?</div>\s*</div>\s*</footer>',
    re.DOTALL
)
...
for page in SUBPAGES:
    ...
    if MINIMAL_FOOTER_PATTERN.search(content):
        new_content = MINIMAL_FOOTER_PATTERN.sub(FULL_FOOTER, content)
        with open(filepath, 'w') as f:
            f.write(new_content)
        print(f"UPDATED: {page}")
    elif 'footer-grid' in content:
        print(f"OK (already has full footer): {page}")
    else:
        print(f"SKIP (no matching footer pattern): {page}")
